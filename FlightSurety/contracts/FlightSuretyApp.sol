...
contract FlightSuretyApp {
    ...
    uint256 constant insuranceAmount = 1 ether;
    
    event InsuranceBuy(uint Id);
    ...
    */   
    ...
    {   
        require(!flightSuretyData.isAirlineRegistered(newAirline), "Already registered.");
        ...
        uint256  registeredAirlineCount = flightSuretyData.noOfRegisteredAirlines();
      if(registeredAirlineCount < 4)
        ...
        else
        {
            ...
            // If  4 airlines present the consensus must be of 50%
            // if the airline has voted before
            if (airlineVotes[newAirline].length != 0)
            {
                for(uint i = 0; i < airlineVotes[newAirline].length; i++)
                {
                    if(airlineVotes[newAirline][i] == msg.sender)
                    {
                        hasVoted = true;
                        break;
                    }
            ...
            airlineVotes[newAirline].push(msg.sender);

            votes = airlineVotes[newAirline].length;
            uint256 votesNeeded = registeredAirlineCount.div(2);
            if (votes == votesNeeded)
            {
                flightSuretyData.registerAirline(newAirline,name);
                success = true;
                delete airlineVotes[newAirline];
            }
        }
    ...
    */  
    function registerFlight
                                ...
                                requireIsOperational
    {
        require(flightSuretyData.isAirlineRegistered(airlineAddress) == true, "Airline is not registered");
    ...
    */  
                                ...
                                
    {
      ...
      function isFlightRegistered
                                ...
                                view returns(bool)
    {
        bytes32 flightKey = keccak256(abi.encodePacked( airline, flightName, timestamp));
                                ...
                                requireIsOperational
    {
        ...
        require(msg.value == insuranceAmount, "Insuranced amount must be 1 ether or less");
...
contract FlightSuretyData {
    ...
    function  noOfRegisteredAirlines() public view returns(uint256);
    function  fund(address airline , uint256 amt) public payable ;
    function  registerFlight(address airlineAddress, string flightName, uint256 timestamp) external;
    ...
    function addToInsurancePolicy(address airline, string flightName, address passenger, uint256 amt) external;
    function withdrawCreditsForInsuree(address passenger) external;
    ...
    function getPassengerCredit(address passangerAddress) external view returns (uint256);
}
