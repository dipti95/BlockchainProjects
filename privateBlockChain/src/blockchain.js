      ...
      try {
        ...
        if (currentTime - time <= 300) {
          if (bitcoinMessage.verify) {
            ...
            resolve(newBlock)
          } else return reject("Not verified")
        }
      ...
      try {
        ...
        for (const block of chain) {
          if (!(await block.validate())) {
            errorLog.push(block + "block is not valid")
          } else {
            if (
              block.height > 0 &&
              block.previousBlockHash !== chain[block.height - 1].hash
            ) {
              errorLog.push(block + "Invalid block")
            }
          }
          resolve(errorLog)
        }
